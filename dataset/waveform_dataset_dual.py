...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, item):
        mixture_path, clean_path, dual_path = self.dataset_list[item].split(" ")
        ...
        dual, _ = sf.read(os.path.abspath(os.path.expanduser(dual_path)))
        dual_l = dual[:,0]
        dual_r = dual[:,1]

        if self.mode == "train":
            # The input of model should be fixed-length in the training.
            # mixture, clean, dual = sample_fixed_length_data_aligned(mixture, clean, self.sample_length)
            return mixture.reshape(1, -1), clean.reshape(1, -1), dual_l.reshape(1, -1), dual_r.reshape(1, -1), filename
        else:
            return mixture.reshape(1, -1), clean.reshape(1, -1), dual_l.reshape(1, -1), dual_r.reshape(1, -1), filename
